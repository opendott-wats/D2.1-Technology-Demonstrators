...
function bzzz() {
  // TODO: make a list of patterns with growing intensity, then pink from value of current data point
  // For now 14: Strong Buzz
  //         13: Soft Buzz
  serial.write(14)
}
...
function mouseDragged() {
  ...
  for (let i = 0; i < Math.round(dst); i) {
    let value = data[position % dataCount]
    if (value > 0) bzzz()
    let s = map(value, 0, 1, 42, 255)
    let d = map(value, 0, 1, 0.5, 20)
    stroke(s)
    fill(s)
    position += 1
    strokeWeight(1)
